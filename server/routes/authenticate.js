...
router.post(constants.API_URL_AUTHENTICATE, function (req, res, next) {
    try {
        ...
        const query = {
             text: 'SELECT usr.id AS "userId", usr.name AS "userName", usr.fullname AS "userFullName", rol."name" as "userRol" '
              ...
              if (err) {
                ...
                if (queryResult && queryResult.rows && queryResult.rows.length > 0) {
                    ...
                    if (token) {
                        ...
                        res.json({ status: 200, token: token, id: queryResult.rows[0].userId, name: queryResult.rows[0].userName, fullname: queryResult.rows[0].userFullName, rol: queryResult.rows[0].userRol });
                    } else {
                        res.json({ status: 200 });
                    }
                } else {
