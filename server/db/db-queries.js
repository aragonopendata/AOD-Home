...
exports.DB_ADMIN_INSERT_ROLE = 'INSERT INTO manager.roles (name, description, active) '
	+ 'VALUES ($1, $2, $3) '
	+ 'RETURNING manager.roles.id';

exports.DB_ADMIN_UPDATE_ROLE = 'UPDATE manager.roles '
	+ 'SET name = $1, description = $2, active = $3 WHERE id = $4';

exports.DB_ADMIN_DELETE_ROLE = 'DELETE FROM manager.roles rol where rol.id = $1';

exports.DB_ADMIN_GET_USERS_ROLE = 'SELECT manager.users.fullname FROM manager.users '
	+ 'WHERE manager.users.id IN (SELECT manager.users_roles.id_user from manager.users_roles where manager.users_roles.id_role = $1);';
